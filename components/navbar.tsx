...
export default function Navbar() {
  ...
  return (
    <motion.nav
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-700 ${isAttached
    ...
    >
      <div className={`container mx-auto px-6 transition-all duration-700 ${isAttached ? "py-10" : "py-5"}`}>
        <div className="flex items-center justify-between">
          ...
          >
            <motion.div
              className={`rounded-full bg-gradient-to-r from-purple-500 to-cyan-500 flex items-center justify-center font-bold text-white transition-all duration-700 ${isAttached ? "w-14 h-14 text-lg" : "w-11 h-11 text-base"
                }`}
              animate={{
          ...
          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            <Button
              ...
              variant="outline"
              className={`border-white/20 text-white hover:border-white/40 transition-all duration-700 font-medium ${isAttached
